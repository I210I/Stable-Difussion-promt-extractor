...
(function (app) {
    ...
    function read_json() {
        ...
        for (let i = 0; i < rows.length; i++) {
            let cell = rows[i].children[3];  
            let ul = cell.querySelector('ul');
            if (ul) {
                let lis = ul.querySelectorAll('li');
                if (lis && lis.length >= 2 && lis[0].textContent.includes("Keyword: prompt") && lis[1].textContent.startsWith("Text string: ")) {
                    let jsonString = lis[1].textContent.replace("Text string: ", "").trim();
                    try {
                        let data = JSON.parse(jsonString);
                        const colors = ["#ed3e35", "#fbd332", "#01a0e9", "#5bb512", "#ff961f", "#009cff",
                            "#ff3d74", "#00bdd6", "#ff5724", "#ff4281", "#f9a824",
                            "#ff5d74", "#ff4ee4", "#fdab77", "#ff80b9", "#00ff30",
                            "#0035cb", "#d80103", "#038802", "#007FFF", "#735cb0"];
                        ...
                        for (let key in data) {
                            ...
                            cardContent.textContent = JSON.stringify(data[key].inputs, null, 2);
        ...
        if(flag){
            Swal.fire({
                title: 'Error!',
                text: 'Maybe your photo has not prompt, but I show your metadata!',
                icon: 'error',
